...
# tm패키지 -> 코퍼스(corpus 말뭉지) 구축

install.packages("tm") # 텍스트마이닝 툴
library(tm)

# corpus? 특징 도메인에서 통용되는 단어 집합
# ex) 법령코퍼스
...
# 불용어 제거
CORPUS <- Corpus(VectorSource(IMDB$Description)) # IMDB$Description에 등장하는 단어 집합으로 코퍼스 생성
...
v <- sort(rowSums(m), decreasing = TRUE)  # 내립차순

names(v)
d <- data.frame(word = names(v), freq = v)

install.packages("SnowballC") # 어근 추출, runs, running, run... => run
library(SnowballC)
...
# sms_corpus[1:3] text에 대한 내용을 확인
