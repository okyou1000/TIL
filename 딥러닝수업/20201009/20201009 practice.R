...
# 
# ex) 점 p가 있을 때, 점 p로부터 eps 이내에 다른 점들이(점 p도 포함됨) minPts개 이상 있다면,
# 이 영역을 하나의 클러스터로 간주
...
teens$female <- ifelse(teens$gender == "F" & !is.na(teens$gender), 1, 0) # F이면서 NA가 아니면 1, 그렇지 않으면 남성(0)
...
# aggregate(data, y(나이)~x(졸업연도), 집계함수(평균), na.rm = TURE)
# x를 기준으로 y에 대해 집계합수를 수행
